...
export interface BuildAppOptions {
  prisma: PrismaClient;
  // 测试时显式传 null 可跳过 Redis/BullMQ；默认连接 REDIS_URL
...
export async function buildApp(options: BuildAppOptions) {
  ...
  if (options.redisUrl !== null) {
    redis = new Redis(options.redisUrl || process.env.REDIS_URL || 'redis://localhost:6379', {
      maxRetriesPerRequest: null,
    });
    redis.on('error', (error) => app.log.warn({ err: error }, 'Redis connection error'));
    mediaQueue = new Queue('media', { connection: redis });
  }
...
// 在统一错误/404 处理器之后注册，使邀请路由同样走全局错误格式
await app.register(invitationRoutes, { prisma });
...
app.get('/ready', async (_req, reply) => {
  try {
    ...
    if (redis && redis.status !== 'ready') await redis.ping();
    return { ok: true };
  } catch {
...
app.post(
  ...
  async (req, reply) => {
    ...
    await requireMembership(req, prisma, workspaceId, [Role.OWNER, Role.EDITOR]);
    ...
    try {
      ...
      try {
        if (!mediaQueue) {
          throw new Error('media queue is not configured');
        }
...
app.get('/v1/workspaces/:id/recordings', { preHandler: authenticate }, async (req) => {
  const workspaceId = (req.params as { id: string }).id;
  await requireMembership(req, prisma, workspaceId);
...
app.get('/v1/recordings/:id/file', async (req, reply) => {
  ...
  const user = await resolveRequestUser(app, req);
...
app.get('/v1/recordings/:id/clips', { preHandler: authenticate }, async (req) => {
  ...
  await requireMembership(req, prisma, recording.workspaceId);
...
app.post('/v1/recordings/:id/clips', { preHandler: authenticate }, async (req, reply) => {
  ...
  await requireMembership(req, prisma, recording.workspaceId, [Role.OWNER, Role.EDITOR]);
...
app.patch('/v1/clips/:id', { preHandler: authenticate }, async (req, reply) => {
  ...
  await requireMembership(req, prisma, old.workspaceId, [Role.OWNER, Role.EDITOR]);
...
app.post('/v1/chapters/:id/blocks', { preHandler: authenticate }, async (req, reply) => {
  ...
  await requireMembership(req, prisma, chapter.workspaceId, [Role.OWNER, Role.EDITOR]);
