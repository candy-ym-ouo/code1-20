...
export const invitationRoleSchema = z.enum(['EDITOR', 'COMMENTER', 'VIEWER']);
...
export const invitationCreateSchema = z
  .object({
    ...
    // 有效期，单位秒：最短 5 分钟，最长 30 天，默认 7 天。
    ttlSeconds: z.number().int().min(5 * 60).max(30 * 24 * 60 * 60).optional(),
  })
  .strict();
